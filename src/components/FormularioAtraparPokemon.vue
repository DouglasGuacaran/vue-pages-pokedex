        ...
        <b-form>
          <b-form-group class="mt-3 mb-2" label="Tipo Pokémon:">
            <select
              ...
              v-model="pokemon.tipoPokemonnombre"
              id="nombreProducto"
              @change="onChangeTipoPokemon($event)"
            >
              <option
                v-for="(item, index) in listaPokemonTipoOpts"
                :key="index"
                :value="item.display"
              >
                {{ item.display }}
          ...
          <b-form-group
            class="mt-3 mb-2"
            label="Nombre Pokemón:"
            label-for="NombrePokemon"
          >
            <select
              class="custom-select form-select"
              v-model="pokemon.nombrePokemon"
              id="nombreProducto"
              @change="onPokemonSelect($event)"
            >
              <option
                v-for="(item, index) in listaPokemonPorTipoOpts"
                :key="index"
                :value="item.url"
              >
                {{ item.display }}
          ...
          <b-form-group class="mt-3 mb-2" label="Lugar:" label-for="Lugar">
            <b-form-input
              id="Lugar"
              v-model="pokemon.lugar"
              type="text"
            ></b-form-input>
          </b-form-group>
          <button
            @click="guardarPokemonSeleccionado()"
            type="button"
            class="m-3 btn btn-primary button"
          >
            Atrapar
          </button>
        </b-form>
...
export default {
  ...
  methods: {
    ...mapActions([
      "pushArrayRutasAction",
      "ArrayAPIPostPokemonAction",
      "DisplaydeTiposdePokemonAction",
      "getPokemonByType",
      "getSelectedPokemonData",
      "agregarPokemonAlArray",
    ]),
    fetchPokemonEspecifico() {
      const a = this.listaPokemonTipoOpts.filter(
        (elemento) => elemento.display == this.pokemon.tipoPokemonnombre
      );
